...
{
    [self setDefaultImage:[UIImage imageNamed:name]];
}
    ...
    if (_imageUrl != imageUrl) {
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if (!image) {
                ...
                [[WSYImageCache sharedImageCache] storeImage:image forKey:imageUrl];
                [self setImage:image animation:YES];
            }else {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(),^(void) {
        @autoreleasepool {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                ...
                if(image == nil)
                    bluredImage = [self getBlurredImage:[UIImage imageNamed:@"tu4.jpg"]];
                else
...
{
    [self.foreImageView setContentMode:contentMode];
    [self.backImageView setContentMode:contentMode];
}
...
- (UIImage *)getBlurredImage:(UIImage *)imageToBlur {
    ...
    return [imageToBlur applyBlurWithCrop:CGRectMake(0,0,imageSize.width,imageSize.height) resize:imageSize blurRadius:1.0f tintColor:[UIColor clearColor] saturationDeltaFactor:1.0 maskImage:[UIImage imageNamed:@"tu2.jpg"]];
}
